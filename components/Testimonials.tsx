...
const StarRating: React.FC<{ stars: number }> = ({ stars }) => (
  <div className="flex gap-0.5">
    {Array.from({ length: 5 }).map((_, i) => (
      <svg
        key={i}
        className={`w-5 h-5 ${i < stars ? 'text-yellow-400' : 'text-stone-600'}`}
        fill="currentColor"
        viewBox="0 0 20 20"
      >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {TESTIMONIALS.map((t, index) => (
            <div
              key={t.id}
              className={`relative bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col gap-4 hover:bg-white/10 transition-colors
                ${index === 0 ? 'lg:col-span-1 border-yellow-400/40 bg-yellow-400/5' : ''}`}
            >
              {/* Top row */}
              ...
              {/* Quote */}
              <p className="text-stone-200 font-serif italic leading-relaxed flex-grow">
                "{t.quote}"
              </p>
